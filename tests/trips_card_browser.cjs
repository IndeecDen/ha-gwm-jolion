...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(async()=>{
   sample={...sample,km:12.3,method:'gps',moving_seconds:300,segments:[[[55.75,37.60],[55.751,37.601],[55.752,37.602],[55.753,37.603],[55.754,37.604],[55.755,37.605],[55.755,37.605]]],gaps:[],segment_speeds_kmh:[[79.9,80,95,109.9,110,0]],segment_kinds:[['moving','moving','moving','moving','moving','stationary']],parking_spots:[{latitude:55.7525,longitude:37.6015,start:1789579200,end:1789586400,duration:7200},{latitude:55.754,longitude:37.603,start:1789586400,end:null,duration:600}]};
   await card._load();
  });
  ...
  const parkingTooltips=await page.evaluate(()=>card._layer.getLayers().filter(layer=>layer.options?.color==='#455a64'&&layer.getTooltip?.()).map(layer=>layer.getTooltip().getContent().textContent));
  assert.equal(parkingTooltips.length,2);
  assert.match(parkingTooltips[0],/Начало:.*Конец:/s);
  assert.match(parkingTooltips[1],/Начало:/);
  assert.doesNotMatch(parkingTooltips[1],/Конец:/);
  ...
  console.log('Trips card: route, configurable speed ranges, parking markers and times, stationary filtering, centering, wheel zoom, touch zoom, dragging, fullscreen, calendar, visual editor, mobile layout, empty history and errors passed');
