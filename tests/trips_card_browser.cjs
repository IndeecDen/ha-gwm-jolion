...
(async()=>{
 ...
 try{
  ...
  assert.deepEqual(parkingMarkers,['A','A','B']);
  ...
  assert.match(parkingTooltips[1],/Стоянка A.*Начало: \d{2}\.\d{2}\.\d{4}.*Конец:/s);
  assert.match(parkingTooltips[2],/Стоянка B.*Начало: \d{2}\.\d{2}\.\d{4}/s);
  ...
  assert.deepEqual(midnightLabels,['A','B','C','D','A']);
  ...
  console.log('Trips card: route, A/B/C parking points, daily labels, date-aware tooltips, configurable speed ranges, centering, map interactions, fullscreen, calendar, visual editor, mobile layout, empty history and errors passed');
