...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(async()=>{
   sample={...sample,km:12.3,method:'gps',moving_seconds:300,segments:[[[55.75,37.60],[55.751,37.601],[55.752,37.602],[55.753,37.603],[55.754,37.604],[55.755,37.605],[55.755,37.605]]],gaps:[],segment_speeds_kmh:[[79.9,80,95,109.9,110,0]],segment_kinds:[['moving','moving','moving','moving','moving','stationary']],parking_spots:[{latitude:55.7525,longitude:37.6015,start:1789579200,end:1789586400,duration:7200}]};
   await card._load();
  });
  ...
  assert.match(await page.evaluate(()=>card._layer.getLayers().find(layer=>layer.options?.color==='#455a64'&&layer.getTooltip)?.getTooltip().getContent().textContent),/Начало:.*Конец:/s);
