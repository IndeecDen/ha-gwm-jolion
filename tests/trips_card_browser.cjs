...
(async()=>{
 ...
 try{
  ...
  const centerBeforeDrag=await page.evaluate(()=>({lat:card._map.getCenter().lat,lng:card._map.getCenter().lng}));
  const mapBox=await page.locator('.map').boundingBox();
  await page.mouse.move(mapBox.x+mapBox.width/2,mapBox.y+mapBox.height/2);await page.mouse.down();await page.mouse.move(mapBox.x+mapBox.width/2+60,mapBox.y+mapBox.height/2+40,{steps:5});await page.mouse.up();
  await page.waitForFunction(before=>Math.abs(card._map.getCenter().lat-before.lat)>0.000001||Math.abs(card._map.getCenter().lng-before.lng)>0.000001,centerBeforeDrag);
  ...
  console.log('Trips card: route, centering, wheel zoom, touch zoom, dragging, fullscreen, calendar, mobile layout, empty history and errors passed');
