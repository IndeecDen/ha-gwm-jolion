...
(async () => {
  ...
  try {
    ...
    await page.evaluate(()=>{card._entryId='demo-entry';card._entities.seatDriver='sensor.seatDriver';card._entities.seatPassenger='sensor.seatPassenger';hass.states['sensor.seatDriver']={state:'2',attributes:{}};hass.states['sensor.seatPassenger']={state:'0',attributes:{}};card._render();});
    await page.locator('[data-action="seat_heating"]').click();
    assert.deepEqual(await page.evaluate(()=>calls.pop()),['gwm_jolion','set_seat_heating',{entry_id:'demo-entry',driver:0,passenger:0,operation_time:10}]);
    assert.match(await page.locator('[data-action="seat_heating"]').innerText(),/Сиденья вкл/);
    await page.evaluate(()=>{hass.states['sensor.seatDriver'].state='0';card._render();});
    assert.match(await page.locator('[data-action="seat_heating"]').innerText(),/Сиденья выкл/);
    await page.locator('[data-action="seat_heating"]').click();
    assert.deepEqual(await page.evaluate(()=>calls.pop()),['gwm_jolion','set_seat_heating',{entry_id:'demo-entry',driver:3,passenger:3,operation_time:10}]);
