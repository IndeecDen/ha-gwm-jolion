...
(async () => {
  ...
  try {
    ...
    assert.deepEqual(await page.evaluate(()=>calls.pop()),['gwm_jolion','set_seat_heating',{entry_id:'demo-entry',driver:3,passenger:3,operation_time:5}]);
    assert.match(await page.locator('[data-action="seat_heating"]').innerText(),/2 \/ выкл/);
