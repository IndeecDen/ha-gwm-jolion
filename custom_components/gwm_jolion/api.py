...
from .const import (
    APP_ID, APP_KEY, APP_SEC, APP_VERSION, AUTH_PREFIX, BASE_URL, BRAND, COUNTRY,
    ENDPOINT_CHECK_SECURITY_PASSWORD, ENDPOINT_FIND_STATUS, ENDPOINT_LAST_STATUS,
    ENDPOINT_LOGIN, ENDPOINT_MODIFY_REMOTE_CTL_INFO, ENDPOINT_T5_CTRL_RESULT,
    ENDPOINT_T5_SEND_CMD, ENDPOINT_VEHICLE_BASICS_INFO, ENDPOINT_VEHICLES,
    ENTERPRISE_ID, LANGUAGE, REGION_CODE, SYSTEM_TYPE, TERMINAL,
)
...
class GwmJolionApiClient:
    ...
    async def async_update(self) -> dict[str, Any]:
        ...
        return {"vin": vin, "vehicle": car_data, "vehicle_name": vehicle_name, "state": state, "location": location}
    ...
    async def async_send_t5_command(self, vin: str, instructions: dict[str, Any], expected_remote_type: str, security_pin: str | None = None) -> dict[str, Any]:
        await self._ensure_login()
        ...
        _LOGGER.debug("Sending T5 command: seqNo=%s expectedRemoteType=%s instructionKeys=%s type=3 hasSecurityPassword=%s", seq_no, expected_remote_type, list(instructions.keys()), bool(security_password))
        ...
        while time.time() < deadline:
            ...
            for item in matched:
                ...
                if result_code in success_codes:
                    _LOGGER.debug("T5 command succeeded: remoteType=%s code=%s msg=%s", expected_remote_type, result_code, item.get("resultMsg") or "")
                    return item
            ...
            _LOGGER.debug("T5 non-final result: remoteType=%s code=%s msg=%s", expected_remote_type, last_error_code, last_error_msg)
        if last_error_code:
    ...
    async def _get_vehicle_basics_info(self, vin: str) -> dict[str, Any]:
        payload = await self._request("GET", ENDPOINT_VEHICLE_BASICS_INFO, params={"vin": vin, "flag": "true"}, vin_header=vin)
        data = payload.get("data") or {}
        return data if isinstance(data, dict) else {}
    ...
    async def _request(self, method: str, path: str, *, params: dict[str, str] | None = None, body: dict[str, Any] | None = None, with_token: bool = True, vin_header: str | None = None, retry_auth: bool = True) -> dict[str, Any]:
        ...
        try:
            async with self._session.request(method, url, headers=headers, data=body_json.encode("utf-8") if body is not None else None, timeout=30) as response:
