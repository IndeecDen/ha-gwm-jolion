...
> [!WARNING]
> Проект не связан с Great Wall Motor, HAVAL или официальным приложением GWM. Используется неофициальный облачный API. Текущая версия — **0.1.0-alpha.19.5**.
...
В `0.1.0-alpha.19.5` карточка-пульт снова переработана после реальной проверки на узком Lovelace-экране. Центральная сцена стала плотнее и крупнее: автомобиль занимает почти всю доступную ширину, подписи вынесены поверх сцены, а статусные плитки и органы управления визуально разделены.

Ориентация теперь зафиксирована в соответствии с фактическим макетом пользователя:
...
ось петель всех четырёх дверей = левая кромка своей SVG-створки
...
- автомобиль заметно увеличен относительно `alpha.19.4`, чтобы центральная сцена не терялась в пустом фоне;
- показывает отдельную анимацию каждой из четырёх дверей по индивидуальным door binary sensors;
- подписи дверей расположены по сторонам машины: правая сторона сверху, левая снизу;
- плавно открывает отдельный SVG-слой багажника справа;
- при работающем двигателе включает оранжевую пульсацию моторного отсека и engine glyph;
- при включённом климате вращает вентилятор и показывает движущиеся синие потоки воздуха в салоне;
- показывает **Онлайн / Оффлайн** T-Box и 4-сегментный GSM-индикатор в правом верхнем углу;
- выводит отдельные читаемые статусные плитки: двигатель, климат, топливо, пробег, двери, окна, багажник и замок;
- если доступны четыре индивидуальных датчика дверей, статус дверей показывает число реально открытых створок;
- различает закрытый и разблокированный центральный замок визуально, но не выдаёт его за отдельный подтверждённый статус охраны;
- для подтверждённого водительского окна `2210001` показывает три состояния: `1=закрыто`, `3=частично открыто`, `2=полностью открыто`; водительское окно находится снизу спереди, поскольку это передняя левая дверь;
- для остальных окон до завершения физического marker-теста не угадывает конкретную позицию и использует только общий alert открытых окон;
- фары остаются отдельным визуальным слоем и не привязаны к двигателю; автоматическое включение будет добавлено только после подтверждения telemetry-кода света;
...
- кнопки управления;
- дополнительные информационные показатели;
- статусные плитки;
- название карточки;
- цвет автомобиля;
- подтверждение удалённых команд.

По умолчанию выводятся кнопки замка, двигателя, климата, багажника и обновления. Можно дополнительно включить окна, обогрев руля, заднего стекла, передний defrost, панораму и шторку.
...
На узком экране восемь статусных плиток автоматически перестраиваются в сетку 4×2, а кнопки управления — в компактную сетку 3 колонки.
...
Для обоих передних сидений принята одинаковая шкала live-кодов:
...
`vehicleBasicsInfo.leftFrontSeat/rightFrontSeat` оказались сохранёнными preset/config значениями, а не live-state. Они больше не используются как fallback живого подогрева.

Окна оказались многоуровневыми. Generic binary decoder теперь использует консервативную логику:
...
Для `2210001` физически подтверждено: переднее левое / водительское, `1=закрыто`, `3=частично открыто`, `2=полностью открыто`. Позиции `2210002..2210004` будут закреплены после отдельного marker-теста.
...
1. физически выставить одно конкретное состояние автомобиля;
2. добавить соответствующую метку, например `WINDOW_FL_FULL`;
3. нажать **Обновить данные**;
4. перейти к следующему состоянию.

Между меткой и обновлением не следует менять несколько функций одновременно.
