...
Версия **0.1.0-beta.12.1** · [Изменения](CHANGELOG.md) · [Планы](ROADMAP.md)
...
Новые записи накапливаются с частотой опроса GPS. Карточка также читает сохранившуюся историю сущностей из Recorder Home Assistant: местоположение берётся из `device_tracker`, пробег — из датчика одометра того же автомобиля. Датчик определяется автоматически или выбирается в редакторе карточки. Записи до начала локального сбора дополняют маршрут без двойного подсчёта расстояния. Удалённую Recorder историю восстановить нельзя.

Локальное хранение — по умолчанию 90 дней; в настройках интеграции можно выбрать 30–365 дней. Пробег считается по доступным показаниям одометра, при их отсутствии — приблизительно по GPS. Пропуски данных и отсутствие показаний у полуночи могут уменьшать учтённое расстояние. Длинные разрывы маршрута не соединяются линией.
