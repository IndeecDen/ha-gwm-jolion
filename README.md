...
Начиная с `0.1.0-alpha.19.3` карточка-пульт получила новую SVG-сцену автомобиля **вид сверху**: капот слева, багажник справа, водительская сторона сверху. Цвет кузова по-прежнему задаётся в визуальном редакторе.

Новая визуализация:

- мягко подсвечивает автомобиль по состоянию замка: закрыт / разблокирован / нет данных;
- показывает **Online / Offline** T-Box в правом верхнем углу;
- ниже отображает 4-сегментный GSM-индикатор;
- под автомобилем выводит компактные уведомления: двигатель, климат, топливо, пробег, двери, окна и багажник;
- анимирует каждую дверь по отдельным door binary sensors;
- визуально открывает багажник;
- показывает пульсацию моторного отсека при работающем двигателе;
- показывает движущиеся потоки воздуха при включённом климате;
...
- содержит отдельный SVG/CSS-слой фар, но он намеренно не включён до подтверждения telemetry-кода света;
...
На мобильном экране нижние статусы автоматически переходят в компактный режим с иконками.
