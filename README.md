...
> [!WARNING]
> Проект не связан с Great Wall Motor, HAVAL или официальным приложением GWM. Используется неофициальный облачный API. Текущая версия — **0.1.0-alpha.19**.
...
Автоматические capture-записи не включают VIN, точные координаты, телефон/аккаунт, пароль, PIN, access token, IMSI/ICCID, vehicle ID и другие device identifiers.
...
Экспорт ограничен 2000 корректными записями и содержит служебные поля `records_in_file`, `records_exported`, `truncated`, `invalid_lines` и `read_error`, чтобы было видно, вошла ли сессия целиком.
