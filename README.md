...
Версия **0.1.0-beta.11** · [Изменения](CHANGELOG.md) · [Планы](ROADMAP.md)

В **Настройки → Устройства и службы → GWM Jolion → Настроить** доступен отдельный ползунок **«Интервал обновления GPS»** (30–3600 секунд). Например, GPS можно обновлять каждые 30 секунд, а остальные данные — раз в 5 минут. При обновлении используется прежний интервал опроса, для новой установки — 5 минут. Частый опрос не заставляет автомобиль чаще передавать координаты в облако.
...
Добавьте карточку **«GWM Jolion — Поездки»** и выберите сущность местоположения своего автомобиля в её графическом редакторе. Кнопки «Сегодня», «Вчера», «7 дней» и календарь позволяют просматривать маршрут и пробег за выбранные даты. День считается по часовому поясу Home Assistant, от полуночи до следующей полуночи.
...
История накапливается после установки, с частотой опроса GPS. Хранение локальное, по умолчанию 90 дней; в настройках интеграции можно выбрать 30–365 дней. Пробег считается по доступным показаниям одометра, при их отсутствии — приблизительно по GPS. Пропуски данных и отсутствие показаний у полуночи могут уменьшать учтённое расстояние. Длинные разрывы маршрута не соединяются линией.

Для подложки карты нужен интернет: браузер загружает видимую область OpenStreetMap. История маршрутов остаётся в Home Assistant и не включается в диагностику. [Подробнее об истории и точности](docs/trips.md).
