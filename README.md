...
Версия **0.1.0-beta.2** · [Изменения](CHANGELOG.md) · [Планы](ROADMAP.md)
...
Интеграция последовательно запускает двигатель, климат и подогревы, проверяя ответы GWM. Между командами сохраняется настроенная пауза; это не строго одновременное включение. При ошибке дальнейшие этапы прекращаются, уже выполненные команды не отменяются. Режим подготовки действует в текущей карточке; после перезагрузки страницы его нужно включить снова.
