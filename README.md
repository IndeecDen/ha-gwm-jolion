...
> [!WARNING]
> Проект не связан с Great Wall Motor, HAVAL или официальным приложением GWM. Используется неофициальный облачный API. Текущая версия — **0.1.0-alpha.19.4**.
...
В `0.1.0-alpha.19.4` карточка-пульт полностью перерисована в виде горизонтальной анимированной сцены автомобиля сверху: **капот слева, багажник справа, левая сторона автомобиля сверху, правая снизу**. Цвет кузова по-прежнему задаётся в визуальном редакторе.

Ориентация дверей зафиксирована явно и защищена regression-тестом:

```text
верх сцены = левая сторона автомобиля
низ сцены  = правая сторона автомобиля

FL = передняя левая  = сверху спереди
RL = задняя левая    = сверху сзади
FR = передняя правая = снизу спереди
RR = задняя правая   = снизу сзади

ось петель всех четырёх дверей = левая кромка своей двери
FL/RL открываются наружу вверх
FR/RR открываются наружу вниз
```

Новая визуализация:

- показывает отдельную анимацию каждой из четырёх дверей по индивидуальным door binary sensors;
- плавно открывает отдельный SVG-слой багажника и подсвечивает его красным;
...
- выводит внизу восемь компактных статусов: двигатель, климат, топливо, пробег, двери, окна, багажник и замок;
...
- для подтверждённого водительского окна `2210001` показывает три состояния: `1=закрыто`, `3=частично открыто`, `2=полностью открыто`;
- для остальных окон до завершения физического marker-теста не угадывает конкретную позицию и показывает общий alert открытых окон;
- содержит отдельный SVG/CSS-слой фар, но он намеренно не активируется до подтверждения telemetry-кода света;
- капот пока остаётся статическим, потому что отдельный подтверждённый hood-status ещё не найден.
...
- нижние статусы/уведомления;
- название карточки;
- цвет автомобиля;
- подтверждение удалённых команд.

По умолчанию выводятся три основные кнопки: замок, двигатель и климат. Можно добавить багажник, окна, обновление, обогрев руля, заднего стекла, передний defrost, панораму и шторку.
...
На узком экране восемь нижних статусов автоматически перестраиваются в компактную сетку 4×2.
