...
Этот раздел имеет приоритет над `alpha.19.4`, если историческое описание ориентации ниже ему противоречит.

- ✅ `custom:gwm-jolion-remote-card` уплотнена для реального узкого Lovelace-экрана: машина заметно крупнее и занимает почти всю центральную ширину;
...
- ✅ у всех четырёх дверей `transform-origin` остаётся на левой кромке своей SVG-створки;
- ✅ FR/RR открываются наружу вверх, FL/RL — наружу вниз;
- ✅ door callouts вынесены поверх сцены и стали читаемее на узкой карточке;
- ✅ отдельные door animations используют четыре существующих door binary sensors;
- ✅ regression-test теперь жёстко фиксирует FR/RR сверху и FL/RL снизу, entity mapping и ось петель;
- ✅ багажник анимируется отдельным слоем справа;
- ✅ работающий двигатель имеет отдельный оранжевый animated overlay;
- ✅ климат имеет вращающийся вентилятор и animated airflow;
- ✅ статусы engine / climate / fuel / mileage / doors / windows / trunk / lock отделены от кнопок управления;
- ✅ T-Box Online/Offline и GSM остаются справа сверху;
- ✅ на узком экране статусы перестраиваются в 4×2, controls — в 3 колонки;
- 🟢 подтверждённое водительское окно `2210001` находится на FL, то есть снизу спереди; шкала `1=closed`, `3=partial`, `2=full open`;
- ⏳ физически закрепить позиции `2210002..2210004` отдельным marker-тестом и только после этого анимировать каждое оставшееся окно отдельно;
- ⏳ фары: не привязывать к двигателю; подключить анимацию только после физического подтверждения `2204007/2204008`;
...
Исторический раздел. Его прежняя ориентация была исправлена в `alpha.19.5` после проверки карточки на реальном экране.

- ✅ `custom:gwm-jolion-remote-card` полностью перерисована в горизонтальный top-view: капот слева, багажник справа;
- ✅ в `alpha.19.4` верх SVG считался левой стороной автомобиля, низ SVG — правой; **это заменено правильной ориентацией в `alpha.19.5`**;
- ✅ у всех четырёх дверей SVG `transform-origin` находится на левой кромке своей двери;
...
- ✅ климат имеет вращающийся вентилятор и animated airflow;
- ✅ нижняя строка статусов: engine / climate / fuel / mileage / doors / windows / trunk / lock;
...
- ✅ `alpha.19.5`: большой горизонтальный вид сверху, капот слева / багажник справа;
- ✅ `alpha.19.5`: верх = правая сторона (FR/RR), низ = левая (FL/RL);
- ✅ `alpha.19.5`: левая ось петель каждой SVG-створки закреплена regression-тестом;
- ✅ изменяемый цвет кузова;
- ✅ locked/unlocked glow;
- ✅ Online/Offline + 4-сегментный GSM справа сверху;
- ✅ status tiles engine/climate/fuel/mileage/doors/windows/trunk/lock;
...
- ✅ mobile layout: statuses 4×2, controls 3 columns;
...
- ✅ `alpha.19.5`: regression-test ориентации FR/RR сверху, FL/RL снизу, петель и driver-window placement;
...
Remote-card скорректирована после проверки на реальном узком Lovelace-экране: автомобиль увеличен, пустое пространство сокращено, подписи дверей сделаны читаемее. Правильная ориентация сцены теперь закреплена тестами: капот слева, багажник справа, FR/RR сверху, FL/RL снизу, ось каждой двери на её левой кромке. Статусы отделены от controls. Новых GWM remote payload и telemetry mappings нет.

### `0.1.0-alpha.19.4` — Animated horizontal remote card (historical)

Первый крупный horizontal top-view redesign. Ориентация верх/низ этого релиза была скорректирована в `alpha.19.5`; остальные основные animated layers (doors/trunk/engine/climate, Online/GSM, statuses) сохранены как база.
